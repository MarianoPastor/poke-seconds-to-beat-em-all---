...
class Character(pygame.sprite.Sprite):
    def __init__(self, sprite_groups, dictionary_surfaces, life, speed, sound_attack, sound_damage, sound_life_gain, character_size, center_x, center_y):
        ...
        self.frame = 0
        self.image = pygame.transform.scale(self.dictionary_surfaces[self.movement_image][self.frame],character_size)
    ...
    def generate_sound(self,path: str, volume: float= 0.6)->None:
    ...
    def change_frame(self):
        self.frame += 1
        if self.frame > len(self.dictionary_surfaces[self.movement_image]):
            self.frame = 0

    def correction_of_directory(self):
        try:
